...
// ==================== التحقق من الصلاحيات (مع قاعدة البيانات) ====================
async function checkUserPermission(req, requiredPermission) {
...
// ==================== السيارات والمصانع والمنتجات (مع صلاحيات) ====================
app.get('/api/trucks', authorize('view_trucks'), async (req, res) => {
    const settings = await loadSettings();
    res.json(settings.trucks || []);
});
...
app.get('/api/truck-violations/stats/:startDate/:endDate', authorize('view_failed_trucks'), async (req, res) => {
    try {
        ...
        const zeroTripsCount = allViolations.filter(v => v.trips_count === 0).length;
        ...
        res.json({ general: { total_trucks: uniqueTrucksViolated, total_violations: totalViolations, avg_trips: avgTrips, zero_trips_count: zeroTripsCount }, topTrucks, topReasons });
