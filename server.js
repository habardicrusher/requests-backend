...
async function initTables() {
    try {
        ...
        `);
        // إضافة العمود إذا كان الجدول موجوداً مسبقاً ولكن بدون العمود
        ...
        // المستخدمون الافتراضيون (مع صلاحيات افتراضية)
        ...
        if (adminCheck.rows.length === 0) {
            ...
            await query(
                ...
                ['admin', bcrypt.hashSync('admin', 10), 'admin', JSON.stringify(defaultAdminPermissions)]
            );
            const defaultUserPermissions = {
                viewOrders: true, addOrders: true, editOrders: false, deleteOrders: false,
                viewDistribution: true, manageDistribution: false, viewTrucks: true, manageTrucks: false,
                viewReports: true, exportReports: false, viewSettings: false, manageSettings: false,
                viewBackup: false, manageBackup: false, manageUsers: false, manageRestrictions: false
            };
            await query(
                "INSERT INTO users (username, password, role, permissions) VALUES ($1, $2, $3, $4)",
                ['user', bcrypt.hashSync('user', 10), 'user', JSON.stringify(defaultUserPermissions)]
            );
            await query(
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        req.session.permissions = user.permissions || {}; // تخزين الصلاحيات في الجلسة
...
// ★★★ إعادة الصلاحيات مع بيانات المستخدم ★★★
app.get('/api/me', async (req, res) => {
    ...
    const user = result.rows[0];
    // تحديث الجلسة إذا تغيرت الصلاحيات من قاعدة البيانات (نادر)
    if (user.permissions && JSON.stringify(user.permissions) !== JSON.stringify(req.session.permissions)) {
        req.session.permissions = user.permissions;
    }
    res.json({ user: { ...user, permissions: user.permissions || {} } });
});
...
// ==================== باقي endpoints (كما هي) ====================
// ... (كل endpoints تقارير الميزان، المخالفات، القيود، النسخ الاحتياطي تبقى كما هي دون تغيير)
// سأختصرها هنا لأنها طويلة ولكن تبقى نفس الكود الأصلي
